...
public class FinancialProductService {
    ...
    public void toggleBookmarkByUsername(String username, Long productId) {
        ...
        Optional<UserBookmark> existingBookmark = userBookmarkRepository.findByUserAndFinancialProduct(user, product);

        if (existingBookmark.isPresent()) {
            // 북마크가 이미 존재하면 제거하고, 북마크 수 감소
            userBookmarkRepository.delete(existingBookmark.get());
            product.setBookmarkCount(product.getBookmarkCount() - 1); // 북마크 수 감소
        } else {
            // 북마크가 없으면 추가하고, 북마크 수 증가
            UserBookmark newBookmark = new UserBookmark(user, product);
            userBookmarkRepository.save(newBookmark);
    ...
    public List<FinancialProduct> getBookmarkedFinancialProductsByUsername(String username) {
        User user = userRepository.findByNickname(username)
                .orElseThrow(() -> new IllegalArgumentException("User not found with username: " + username));
        return userBookmarkRepository.findByUsername(username).stream()
                .map(UserBookmark::getFinancialProduct)
                .collect(Collectors.toList());
    }

    // 특정 금융상품 북마크 수 조회
    public long getBookmarkCountForProduct(Long productId) {
        return userBookmarkRepository.countByFinancialProductId(productId);
    }
